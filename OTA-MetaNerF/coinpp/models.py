...
class PositionalEncoding(nn.Module):
    ...
    def forward(self, input):
        output = torch.cat([func(input) for func in self.embed_funcs], -1)
        return output
...
class ModulatedSiren(Siren):
    ...
    def __init__(
        ...
        self.encoding = PositionalEncoding(3, num_freq - 1, num_freq)
    ...
    def noisy_modulated_forward(self, x, latent, snr = 10):
        """Forward pass of modulated SIREN model.
        ...
        # We then calculate the noisy vector
        pwr = torch.mean(latent**2, dim = -1, keepdim=True)      # (batch, 1)
        noise = torch.randn_like(latent)*torch.sqrt(pwr)*10**(-snr/20)

        with torch.no_grad():
            noise_mod = self.modulation_net(noise)

        noisy_modulation = modulations + noise_mod
        ...
        for module in self.net:
            if self.modulate_scale:
                # Shape (batch_size, 1, dim_hidden). Note that we add 1 so
                # modulations remain zero centered
                scale = noisy_modulation[:, idx : idx + self.dim_hidden].unsqueeze(1) + 1.0
            else:
                scale = 1.0

            if self.modulate_shift:
                # Shape (batch_size, 1, dim_hidden)
                shift = noisy_modulation[
                    :, mid_idx + idx : mid_idx + idx + self.dim_hidden
                ].unsqueeze(1)
            else:
                shift = 0.0
